...
class GraphAttention(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        out = torch.matmul(attn, v).permute(0,2,1,3).contiguous().view(B, N, F)
        return out
...
class NASGNNBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, adj: torch.Tensor) -> torch.Tensor:
        ...
        t_inp = g_out.permute(0, 2, 3, 1).reshape(B * N, F, T)  # [BN, F, T]
        t_w   = F.softmax(self.t_arch, dim=0)
        t_out = sum(w * conv(t_inp) for w, conv in zip(t_w, self.t_convs))
        t_out = t_out.reshape(B, N, F, T).permute(0, 3, 1, 2)   # [B, T, N, F]
