...
class UCSDTestDataset(Dataset):
    ...
    def __init__(self, root: str, ped: str = 'ped1',
        ...
        # Build frame list + labels
        # Labels: 0 = normal, 1 = anomaly
        # We use the presence of a _gt directory as the indicator
        self.samples: List[Tuple[List[Path], int]] = []  # (frame_paths, is_anomaly_seq)
        for seq_dir in seq_dirs:
            frames = _get_frame_paths(seq_dir)
            ...
            if clip_len == 1:
                for f in frames:
                    self.samples.append(([f], has_gt))
            else:
                for i in range(len(frames) - clip_len + 1):
                    self.samples.append((frames[i: i + clip_len], has_gt))

    def __len__(self):
        return len(self.samples)

    def __getitem__(self, idx):
        paths, is_anomaly = self.samples[idx]
        imgs = []
        for p in paths:
            fr = _load_frame(p)
            if fr is None:
