...
class BaseTrainer:
    ...
    def __init__(self, model: nn.Module, optimizer, scheduler=None,
        ...
        self.start_epoch = 0
        self._resume_no_improve = 0      # early-stop counter restored on resume
        self._resume_total_epochs = None # original target epoch count
    ...
    def train(self, train_loader, val_loader,
        ...
        """
        best_epoch = 0
        no_improve = self._resume_no_improve
        t0 = time.time()

        # Reset best_metric if it was never set by a checkpoint
        if self.best_metric == float('inf') and not lower_is_better:
            self.best_metric = float('-inf')

        # On resume, train to the original target rather than adding
        # another full `epochs` on top of start_epoch.
        total_epochs = self._resume_total_epochs or (self.start_epoch + epochs)
        self._resume_total_epochs = total_epochs   # persist for next save

        if self.start_epoch >= total_epochs:
            print(f"Already completed {self.start_epoch}/{total_epochs} epochs.")
            return self.history
    ...
    def save_checkpoint(self, filename: str, epoch: int, metrics: Dict,
        ...
        state = {
            ...
            'no_improve': no_improve,
            'total_epochs': getattr(self, '_resume_total_epochs', None),
        }
        if self.scheduler:
    ...
    def load_checkpoint(self, filename: str = 'best.pt'):
        ...
        self._resume_no_improve = state.get('no_improve', 0)
        self._resume_total_epochs = state.get('total_epochs', None)
        print(f"Resuming from epoch {self.start_epoch}, "
              f"best={self.best_metric:.4f}, "
              f"no_improve={self._resume_no_improve}")
