...
class FutureFrameNet(nn.Module):
    ...
    def reconstruction_error(self, clip: torch.Tensor) -> torch.Tensor:
        """
        Anomaly score for evaluation.  Takes a full clip [B, T, C, H, W],
        uses frames 0..T-2 as past context, predicts frame T-1, and
        returns per-sample prediction error.

        Also works with pre-flattened [B, T*C, H, W] input from the
        evaluation pipeline (treated as a single-clip prediction).
        """
        if clip.dim() == 5:
            B, T, C, H, W = clip.shape
            past = clip[:, :-1]                  # [B, T-1, C, H, W]
            target = clip[:, -1]                 # [B, C, H, W]
            x = past.reshape(B, (T - 1) * C, H, W)
        else:
            # Already flattened [B, T*C, H, W] — can't split, just predict
            x = clip
            target = clip[:, :1]  # fallback: compare to first channel(s)
        pred = self.forward(x)
